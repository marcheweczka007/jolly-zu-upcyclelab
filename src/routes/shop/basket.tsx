...
import { formatPrice, isPreorder } from "@/data/listings";
...
function BasketPage() {
  const { lineItems, items, removeItem, subtotalPence, clearBasket } = useBasket();
  ...
  const handleCheckout = async () => {
    ...
    setCheckingOut(true);
    const result = await startStripeCheckout(items);
        ...
        {lineItems.length === 0 ? (
          ...
          <>
            <ul className="mt-10 divide-y-2 divide-ink/10">
              {lineItems.map(({ listing }) => (
                <li key={listing.id} className="flex gap-4 py-6 first:pt-0">
                  <Link
                    to="/shop/$listingId"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm text-ink/60">
                      {isPreorder(listing) ? "Pre-order · ships in 2–3 weeks" : "One of a kind"}
                    </p>
                    <button
                      type="button"
                  ...
                  <p className="text-display shrink-0 text-lg">{formatPrice(listing.pricePence)}</p>
                </li>
              ))}
            </ul>
            ...
            <p className="mt-2 text-sm text-ink/55">
              ...
              {lineItems.some(({ listing }) => isPreorder(listing)) &&
