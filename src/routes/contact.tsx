...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Custom orders, collabs, press, or just to say hi — get in touch with JollyZu." },
    ...
    <div className="min-h-screen bg-cream text-ink">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-12 pt-8 md:px-8 md:pb-16 md:pt-16">
        ...
        <h1 className="text-display text-[14vw] leading-[0.85] md:text-[9rem]">
          Say<br />
          <span className="bg-mustard px-3 inline-block -rotate-1">hello.</span>
        </h1>
        <p className="mt-6 max-w-xl text-lg text-ink/70">
          Custom orders, collabs, press, stockists, or just want to chat about fabric? Send a note.
        </p>
      </section>

      <section className="mx-auto max-w-7xl px-5 pb-20 md:grid md:grid-cols-12 md:gap-16 md:px-8 md:pb-32">
        <div className="md:col-span-7">
          <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border-2 border-ink bg-cream p-6 shadow-brutal md:p-10">
            {status === "success" && (
            ...
            <Field label="Your name" error={errors.name}>
              <input name="name" type="text" maxLength={100} required className={inputCls} placeholder="Zuza Smith" />
            </Field>

            <Field label="Email" error={errors.email}>
              <input name="email" type="email" maxLength={255} required className={inputCls} placeholder="you@email.com" />
            </Field>
            ...
            <Field label="Message" error={errors.message}>
              <textarea name="message" rows={6} maxLength={1000} required className={inputCls} placeholder="Tell me what you have in mind…" />
            </Field>
        ...
        <aside className="mt-12 space-y-8 md:col-span-5 md:mt-0">
          ...
          <div className="rounded-2xl border-2 border-ink p-8">
            <p className="text-xs font-bold uppercase tracking-widest text-purple-deep">Faster reply</p>
      ...
      <section className="bg-ink py-24 text-center text-cream md:py-32">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <h2 className="text-display text-5xl md:text-8xl">
            Or just<br /><span className="text-mustard">shop the drop.</span>
          </h2>
